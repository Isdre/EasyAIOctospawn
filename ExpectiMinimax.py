...
def expectiminimaxnature(game, depth, origDepth, scoring, alpha=+inf, beta=-inf, tt=None):
    alphaOrig = alpha
    ...
    for move in possible_moves:
        ...
        if sumValue / len(possible_moves) < move_alpha:
            best_move = move

        sumValue += move_alpha

        if alpha < sumValue / len(possible_moves):
            alpha = sumValue / len(possible_moves)
            # best_move = move
            if depth == origDepth:
                state.ai_move = move
            if alpha >= beta:
    ...
    avgValue = sumValue / len(possible_moves)
    ...
    if tt is not None:
        ...
        tt.store(
            ...
            move=best_move,
            flag=UPPERBOUND
            if (avgValue <= alphaOrig)
            else (LOWERBOUND if (avgValue >= beta) else EXACT),
        )
...
def expectiminimax(game, depth, origDepth, scoring, alpha=+inf, beta=-inf, tt=None):
    ...
    for move in possible_moves:
        ...
        move_alpha = -expectiminimaxnature(game, depth - 1, origDepth, scoring, -beta, -alpha, tt)
